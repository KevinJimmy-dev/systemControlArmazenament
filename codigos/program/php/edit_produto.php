...
if (mysqli_num_rows($exec) >= 2) {
    echo "<script>alert('Esse nome de produto já existe!');</script>";
    echo "<script>window.location.href = 'editar_produto.php?id_produto=$id_produto'</script>";
//Senão, executa o update
} else if ($resultado_produto = mysqli_query($conexaoMysqli, $result_produto)) {
    $_SESSION['msg'] = "<p style='color:green;'>O Produto foi editado com sucesso!</p>"; //vai aparecer essa mensagem...
